...
- **Playwright integration tests for Phaser-coupled paths.** Project policy in CLAUDE.md currently says "Phaser-coupled code (`MusicController.ts`, the scenes) is deferred — needs scene mocking, separate pass." We've been treating that as "skip those tests entirely" and only writing vitest unit tests for pure-TS surfaces. That gap surfaced concretely on `feat/stun-mechanic`: the unit tests with `FakeMainGame` pre-populate `cursors`, so `LeftState.execute()` reading `scene.cursors.up.isDown` passed in vitest — but the real `MainGame.create()` populates `cursors` AFTER the hand setup runs, so calling `handFSM.step()` in `create()` blew up with `Cannot read properties of undefined (reading 'up')` on first browser load. Playwright (or any real-browser integration test that spins up Phaser end-to-end) skips the mocking problem entirely by running the real scene lifecycle, which is the natural place ordering-of-init bugs live.
  - **Scope thinking for when this lands.** This is bigger than "install playwright and write a test" — it's a real test stack addition with decisions worth /office-hours scoping: which browser engine (chromium-only vs cross-browser), headless config (CI default vs local visual debugging), screenshot/video evidence strategy (none / on-failure / always), where the tests live (`src/e2e/` vs `tests/` vs a separate workspace), how they wire into `bun run test` (replace vitest? run alongside? separate `bun run test:e2e`?), CI integration (GitHub Actions workflow + cached browser install), and which initial scenarios to cover (smoke-create-MainGame is the minimum; full set would be hand movement, settings nav, win, timer expiry, stun bounce, suspicion overflow).
  - **Minimum viable.** Even just ONE test — "MainGame creates without error and runs for 2 seconds" — would have caught this specific bug. That smaller scope is also a reasonable first commit if the user wants Playwright incrementally rather than fully.
  - **Trigger to revisit.** The next time a Phaser-coupled ordering bug surfaces in playtest, or before /ship-ing any feature that adds new scene-init coupling (e.g., the stash-spots + look-at-table cluster bundles enough new state to be a strong candidate).
